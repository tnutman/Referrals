...
public class UpdateMember extends HttpServlet {
	...
	private void processRequest(HttpServletRequest request,
		...
		if (httpSession.getAttribute("u_id") == null) {
			...
			PreparedStatement preparedStatement = dbUtil.getPreparedStatement("UPDATE org_membership SET r_id = ? WHERE (u_id = ? AND o_id = ?);");
			preparedStatement.setInt(1, Integer.parseInt(request.getParameter("role_id")));
			preparedStatement.setInt(2, Integer.parseInt(request.getParameter("user_id")));
			preparedStatement.setInt(3, Integer.parseInt((String) httpSession.getAttribute("active_org")));
			preparedStatement.executeUpdate();
			// set the id of the newly created org to be the active account
			httpSession.setAttribute("user_last_action", "member_permission_updated");
			httpSession.setAttribute("system_message", "User ID:  " + request.getParameter("user_id") + " permissions updated");
